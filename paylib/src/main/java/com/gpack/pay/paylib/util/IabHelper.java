...
public class IabHelper {
     ...
     * @throws NullPointerException context cannot be null
     */
    public IabHelper(Context ctx, String base64PublicKey) {
        if (ctx == null)
            throw new NullPointerException("the context to bind to in-app billing service cannot be null");
    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus,
        ...
        try {
            DU.sd("launch purchase flow ", "Constructing buy intent for product id = " + sku,
                    "item type: " + itemType,
                    "payload:" + extraData);
            ...
            if (oldSkus == null || oldSkus.isEmpty()) {
                ...
                DU.sd("extra data- pay load", extraData);
            } else {
                ...
                // FIXME: 2016/1/13 comment for the unsupported api
                buyIntentBundle = mService.getBuyIntentToReplaceSkus(5, mContext.getPackageName(),
                        oldSkus, sku, itemType, extraData);
            }
            int response = getResponseCodeFromBundle(buyIntentBundle);

            DU.sd("get response result", response, "old sku:" + (oldSkus == null ? null : oldSkus.size()),
                    "new sku:" + sku);
     ...
     * post to server
     */
    private void postToServer(String purchaseValue, String signatureValue, String publishKeyValue) {
        final String verifyDataUrl = PayURL.HOST + PayURL.VERIFY +
                PayURL.PUBLISH_KEY + publishKeyValue;

        // verified ok, post to server
        PayDelegate.getThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                String verifyResult = HttpUtils.doPost(verifyDataUrl, null);

                DU.sd("http opera", "url = " + verifyDataUrl, "result = " + verifyResult);

                try {
                    if (TextU.hasContent(verifyResult)) {
                        JSONObject object = new JSONObject(verifyResult);
                        int status = JsonUtils.getInt(object, "status", -1);
    ...
    public void queryInventoryAsync(final List<String> moreSkus, QueryInventoryFinishedListener listener) {
        queryInventoryAsync(true, moreSkus, listener);
    }

    public void queryInventoryAsync(QueryInventoryFinishedListener listener, final List<String> moreSkus) {
        queryInventoryAsync(true, moreSkus, listener);
    }
